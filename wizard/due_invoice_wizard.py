...
class DueInvoiceWizard(models.TransientModel):
    ...
    def send_report_email(self):
        ...
        attachment = self.env["ir.attachment"].create({
            "name": f"Statement Of Accounts: {self.partner_id.name}.pdf",
            "type": "binary",
            "datas": pdf_base64,
...
class DueInvoiceWizard(models.TransientModel):
    ...
    def send_report_email(self):
        """Generates the Due Invoice report and sends it via email if the partner has an email."""
        self.ensure_one()
    ...
    @api.model
    def cron_send_due_invoice_reports(self):
        """Scheduled action to send due invoice reports to all customers with outstanding invoices."""
        partners = self.env["res.partner"].search([("customer_rank", ">", 0), ("email", "!=", False)])

        for partner in partners:
            wizard = self.create({"partner_id": partner.id})
            wizard.send_report_email()
